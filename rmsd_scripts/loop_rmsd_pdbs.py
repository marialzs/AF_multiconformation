...
shape = [1 if pdb[19:-4] in bound_info else 0 for pdb in new_pdb_list] #for bound shape 1

#save scores in order
#scores = pd.read_csv(args.path + '/structure_bs_strength.csv')
scores = pd.read_csv(args.path + '/structure_bs_strength.csv')
colormap = [scores[scores['Structure'] == pdb[19:-4]]['Binding Site 00' + args.site].iloc[0] + (23 if pdb[:-4] == '1W50.A' else 0) if not scores[scores['Structure'] == pdb[19:-4]].empty else 0 for pdb in new_pdb_list]
print(colormap)
#make figure
fig, ax = plt.subplots()
ax.set_box_aspect(1)
...
norm = plt.Normalize(vmin=min(colormap), vmax=max(colormap))
...
# Add a colorbar
sm = plt.cm.ScalarMappable(cmap=cmap, norm=plt.Normalize(vmin=0, vmax=max(colormap)))
sm.set_array([])
cbar = plt.colorbar(sm,ticks = np.arange(0,np.max(colormap)+1,5))
cbar.ax.tick_params(labelsize=14)

#Add labels and show the plot
plt.xlabel('Loop RMSD to Closed Structure (Å)', fontsize = 16)
plt.ylabel('Loop RMSD to Open Structure (Å)', fontsize = 16)
...
for index, (xi, yi) in enumerate(zip(distance_to_closed, distance_to_open)):
    distance_x = math.sqrt((3.8 - xi) ** 2 + (0 - yi) ** 2)
    distance_y = math.sqrt((0 - xi) ** 2 + (3.8 - yi) ** 2)
    if distance_x < 5.37/2:
        open_str.append(new_pdb_list[index])

    elif distance_y < 5.37/2:
